...
class Neuron(DifferentiableElement):
    ...
    def compute_dot_product(self):
        inputs_vector = self.get_inputs()
        weigths_vector = self.get_weights()
        dot_product_sum = 0
        for i in range(len(inputs_vector)):
            dot_product_sum += weigths_vector[i].my_value * inputs_vector[i].output()
    ...
    def compute_output(self):
        ...
        returns: number (float or int)
        """
        return sigmoid(self.compute_dot_product())
    ...
    def compute_doutdx(self, elem):
        ...
        """
        input_derivative = sigmoid_derivative(self.compute_dot_product())
        
        if self.has_weight(elem):
            return self.my_inputs[self.my_weights.index(elem)].output() * input_derivative

        for i in range(len(self.my_weights)):
            weight = self.my_weights[i]
            if self.isa_descendant_weight_of(elem, weight):
                return self.my_inputs[i].dOutdX(elem) * weight.get_value() * input_derivative
        # sum_of_derivatives = 0
        # for i in range(len(self.my_weights)):
        #     weight = self.my_weights[i]
        #     if self.isa_descendant_weight_of(elem, weight):
        #         sum_of_derivatives += self.my_inputs[i].dOutdX(elem) * weight.get_value()
        # return sum_of_derivatives * input_derivative
...
class RegularizedPerformanceElem(PerformanceElem):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        return (-0.5 * np.power(self.my_desired_val - self.my_input.output(), 2)) - self.regularization_rate * self.sum_of_network_weigths()
        # raise NotImplementedError("Implement me!")
    ...
    def dOutdX(self, elem):
        ...
        returns: number (int/float)
        """
        return (self.my_desired_val - self.my_input.output()) * self.my_input.dOutdX(elem) - (2 * self.regularization_rate * elem.my_value)
        # raise NotImplementedError("Implement me!")
    ...
    def sum_of_network_weigths(self):
        sum_of_weights = 0
        weights = self.network.weights
        for i in range(len(weights)):
            sum_of_weights += np.power(weights[i].get_value(), 2)

        return sum_of_weights

def separate_x_y(coordinates):
...
def make_name(src, dest, indexes):
    name = 'w' + src + 'A' + str(indexes[0]) + str(indexes[1]) + dest
    if indexes[2] == 3:
        if indexes[1] == 9:
            indexes[0], indexes[1] = indexes[0] + 1, 0
        else:
            indexes[1] += 1
        indexes[2] = 0
    else:
...
def make_neural_net_two_moons():
    ...
    Neurons = []
    for i in range(40):
        w1 = Weight(make_name('1', '', indexes), random_weight())
        w2 = Weight(make_name('2', '', indexes), random_weight())
        w3 = Weight(make_name('', '', indexes), random_weight())

        Neurons.append(Neuron(make_name('', '', indexes)[1:], [i1, i2, i0], [w1, w2, w3]))

    indexes[0], indexes[1] = 0, 0
    Weights = []
    for i in range(40):
        indexes[2] = 3
        Weights.append(Weight(make_name('', 'B', indexes), random_weight()))
    Weights.append(Weight('wB', random_weight()))

    B = Neuron('B', Neurons + [i0], Weights)

    P = PerformanceElem(B, 0.0)
    # P = RegularizedPerformanceElem(B, 0.0)

    net = Network(P, Neurons + [B])
    # P.set_network(net)

    return net

def plot_decision_boundary(network, xmin, xmax, ymin, ymax):
    bigger_than_half, less_than_half = [], []

    for x in np.arange(xmin, xmax, (xmax - xmin) / 200):
        for y in np.arange(ymin, ymax, (ymax - ymin) / 200):
            network.inputs[0].set_value(x)
            network.inputs[1].set_value(y)
            network.clear_cache()

            if network.output.output() > 0.5:
                bigger_than_half.append([x, y])
            else:
                less_than_half.append([x, y])

            network.clear_cache()

    x, y = separate_x_y(bigger_than_half)
    plt.scatter(x, y, color="lightblue", alpha=0.1)
    x, y = separate_x_y(less_than_half)
...
def train(network,
    ...
    # finite difference 
    if finite_difference(network):
        print("Finite Difference Verified the weights")
    else:
        print("Finite Difference rejected the weights")
...
def finite_difference(network):
    ...
    for i in range(len(network.weights)):
        ...
        old_weight = network.weights[i].get_value()
        network.weights[i].set_value(network.weights[i].get_value() + epsilon)
        network.clear_cache()
        fxPlusEpsilon = network.performance.output()
        network.weights[i].set_value(old_weight)
        if abs(network.performance.dOutdX(network.weights[i]) - ((fxPlusEpsilon - fx) / epsilon)) > 0.0001:
            return False
    return True
